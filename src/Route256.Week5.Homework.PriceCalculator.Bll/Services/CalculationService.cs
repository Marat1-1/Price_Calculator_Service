...
public class CalculationService : ICalculationService
{
    ...
    public async Task<QueryCalculationModel[]> GetCalculations(
        long[] calculationIds,
        CancellationToken token)
    {
        var result = await _calculationRepository.GetCalculations(calculationIds, token);
        return result
            .Select(x => new QueryCalculationModel(
                x.Id,
                x.UserId,
    ...
    {
        using var transaction = _calculationRepository.CreateTransactionScope();
        await _calculationRepository.Delete(calculationIds, token);
        transaction.Complete();
    }

    public async Task DeleteGoods(
        long[] goodIds,
        CancellationToken token)
    {
        using var transaction = _goodsRepository.CreateTransactionScope();
        await _goodsRepository.Delete(goodIds, token);
        transaction.Complete();
    }

    public async Task DeleteAllCalculationsFromUser(
        long userId,
        CancellationToken token)
    {
        using var transaction = _calculationRepository.CreateTransactionScope();
        await _calculationRepository.DeleteAllFromUser(userId, token);
        transaction.Complete();
    }

    public async Task DeleteAllGoodsFromUser(
        long userId,
        CancellationToken token)
    {
        using var transaction = _goodsRepository.CreateTransactionScope();
        await _goodsRepository.DeleteAllFromUser(userId, token);
        transaction.Complete();
    }
}